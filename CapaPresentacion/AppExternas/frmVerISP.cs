...
namespace CapaPresentacion.Partes_Trabajo
{
    public partial class frmVerISP : Form
    {
                ...
                try
                {
                    if (dgvAccounts.Rows.Count > 0)
                    {
                        ...
                        if (dtDatosConexion.Rows.Count > 0)
                        {
                            ...
                            dgvDatosConexion.Columns["customer_id"].Visible = false;
                            dgvDatosConexion.Columns["username"].Visible = false;
                            dgvDatosConexion.Columns["protocolo_conexion"].HeaderText = "Protocolo";
                            dgvDatosConexion.Columns["protocolo_conexion"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                        }
                        int contador = 0;
        ...
        private Boolean Desconectar(int idAccess)
        {
            ...
            string ipRedacct, userName;
            ipRedacct = oISP.GetIpRedacct(idAccess);
            if (!string.IsNullOrEmpty(ipRedacct))
            {
                userName = dgvAccounts.SelectedRows[0].Cells["username"].Value.ToString();
        ...
        private void dgvDatosConexion_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dgvDatosConexion.Rows[e.RowIndex].Cells["ip_conexion"].Value.ToString().StartsWith("10."))
            {
                dgvDatosConexion.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.Tomato;
            }
            else
            {
                dgvDatosConexion.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.LimeGreen;
        ...
        private void btnCortar_Click(object sender, EventArgs e)
        {
            ...
            frmResponsable oResponsable = new frmResponsable();
            oPop.Formulario = oResponsable;
            oPop.Maximizar = false;
            if (oPop.ShowDialog() == DialogResult.OK)
            {
                //103 es el prodcuto cortado en el isp
                bloquearControles();
                oAppExterna.CambiarProducto(idAccountSeleccionado, 103, Convert.ToInt32(dgvAccounts.SelectedRows[0].Cells["product_id"].Value));
                oISP.CambiarProducto(idAccountSeleccionado, Convert.ToInt32(dgvAccounts.SelectedRows[0].Cells["product_id"].Value)); 
                Desconectar(idAccountSeleccionado);
                desbloquearControles();
            }
        }
        ...
        private void btnReconectar_Click(object sender, EventArgs e)
        {
            ...
            oPop.Formulario = oResponsable;
            oPop.Maximizar = false;
            if (oPop.DialogResult == DialogResult.OK)
            {
                bloquearControles();
                oAppExterna.CambiarProducto(idAccountSeleccionado,Convert.ToInt32(dgvAccounts.SelectedRows[0].Cells["product_id"].Value),103);
                oISP.CortarServicio(idAccountSeleccionado);
                Desconectar(idAccountSeleccionado);
                desbloquearControles();
            }
        }
        ...
        {
            dtDatosConexion = oISP.ListarDatosConexion(idAccountSeleccionado);
            if (dtDatosConexion.Rows.Count > 0)
            {
                dgvDatosConexion.DataSource = dtDatosConexion;
                foreach (DataGridViewColumn item in dgvDatosConexion.Columns)
        ...
        private void boton1_Click(object sender, EventArgs e)
        {
            try
            {
                int index = dgvDatosConexion.SelectedCells[0].RowIndex;
                string ipPing = dgvDatosConexion.Rows[index].Cells["ip_conexion"].Value.ToString();
